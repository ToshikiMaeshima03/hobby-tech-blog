...
フロントエンドは以下の4段階で構成されます。以下のダイアグラムはコンパイラの全体アーキテクチャと各フェーズの役割を示しています。
...
flowchart LR
    ...
    style A fill:#3b0764,stroke:#9333ea,color:#f3e8ff
    style J fill:#3b0764,stroke:#9333ea,color:#f3e8ff
    style G fill:#7e22ce,stroke:#c084fc,color:#f3e8ff
```

フロントエンド（字句解析〜IR生成）を自作すれば、最適化とコード生成はLLVMが担当します。
